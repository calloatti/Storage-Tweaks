...
namespace Calloatti.StorageTweaks
{
  ...
  public static class StorageCapacityCache
  {
    ...
    private static readonly string LegacyConfigPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Timberborn", "Mods", "Storage Tweaks", "StorageTweaks.txt");
    private static readonly string ConfigPath = Path.Combine(PlayerDataFileService.PlayerDataDirectory, "StorageTweaks.txt");
    ...
    private static void ProcessConfig(object specService)
    {
      try
      {
        ...
        {
          Debug.LogWarning("[StorageTweaks] Config file missing. Generating default config in PlayerData...");
        }
    ...
    private static void MigrateLegacyConfig()
    {
      try
      {
        ...
        {
          // Check if it's already a dummy redirect file
          string firstLine = string.Empty;
          using (StreamReader reader = new StreamReader(LegacyConfigPath))
          {
            firstLine = reader.ReadLine();
          }

          if (firstLine != null && firstLine.StartsWith("Config file moved"))
          {
          ...
          if (!File.Exists(ConfigPath))
          {
            ...
            if (DebugMode) Debug.Log($"[StorageTweaks] Migrated legacy config to {ConfigPath}");
          }

          File.WriteAllText(LegacyConfigPath, $"Config file moved to:\n{ConfigPath}\n");
        }
      }
      catch (Exception ex)
      {
        Debug.LogError($"[StorageTweaks] Error migrating legacy config: {ex}");
      }
    }
